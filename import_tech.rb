...
if (File.exists?(csv_file))
  ...
  r.each do |row|
    ...
    if Technician.find_by_crm_id(row.crm_id).nil?
      team.technicians.create(:crm_id => row.crm_id, :first_name => row.first_name, :last_name => row.last_name, :friendly_name => row.friendly_name, :email => row.email, :car_stock_number => row.car_stock_number)
    end
  end
else
