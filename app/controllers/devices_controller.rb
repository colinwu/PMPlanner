class DevicesController < ApplicationController
  ...
  before_action :require_manager, only: [:new, :create, :destroy, :unassigned]
  ...
  def do_search
    ...
    if params[:commit] == 'Search' and not params[:search_str].blank?
      ...
      unless current_user.admin?
        active_str = 'and active = true'
      end
      if @target == 'All'
        @devices = Device.joins(:model, :client, :location, :primary_tech).where(["(crm_object_id regexp ? or serial_number regexp ? or models.nm regexp ? or clients.name regexp ? or locations.address1 regexp ? or technicians.first_name regexp ? or technicians.last_name regexp ? or technicians.friendly_name regexp ?) #{active_str}", @search_str, @search_str, @search_str, @search_str, @search_str, @search_str, @search_str, @search_str]).order(:crm_object_id).page(params[:page]).per_page(lpp)
      elsif @target == 'Region'
        @devices = Device.joins(:model, :client, :location, :primary_tech).where(["(crm_object_id regexp ? or serial_number regexp ? or models.nm regexp ? or clients.name regexp ? or locations.address1 regexp ? or technicians.first_name regexp ? or technicians.last_name regexp ? or technicians.friendly_name regexp ?) and devices.team_id = ? #{active_str}", @search_str, @search_str, @search_str, @search_str, @search_str, @search_str, @search_str, @search_str, current_technician.team_id]).order(:crm_object_id).page(params[:page]).per_page(lpp)
      else
  ...
  def device_params
    params.require(:device).permit(:installed_base_id, :crm_object_id, :alternate_id, :model_id, :client_id, :serial_number, :location_id, :primary_tech_id, :backup_tech_id, :active, :under_contract, :do_pm, :pm_counter_type, :pm_visits_min, :notes, :team_id, :install_date, :acctmgr, :crm_active, :crm_under_contract, :crm_do_pm)
  end
end
